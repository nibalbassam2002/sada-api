...
{
    $request->validate([
        'file'  => 'required|file|mimes:pptx,zip|max:20480', // max 20MB
    ...
    try {
        ...
        $tempPath = $file->storeAs('temp_pptx', uniqid() . '.pptx', 'local');
        ...
        foreach ($presentation->getAllSlides() as $slide) {
            ...
            try {
                ...
                if ($bg && method_exists($bg, 'getFillColor')) {
                    $color = $bg->getFillColor();
                    if ($color && $color->getRGB()) {
                        $bgColor = '#' . strtolower($color->getRGB());
                    }
                }
            } catch (\Throwable $e) {}

            // استخراج النصوص من كل shape
            foreach ($slide->getShapeCollection() as $shape) {
                if (!method_exists($shape, 'getActiveParagraphs')) continue;
                ...
                foreach ($shape->getActiveParagraphs() as $paragraph) {
                    ...
                    foreach ($paragraph->getRichTextElements() as $element) {
                        if (method_exists($element, 'getText')) {
                            $paraText .= $element->getText();
                        }
                    }
                    if (trim($paraText)) {
                        $fullText .= trim($paraText) . "\n";
                    }
                }
                ...
                // أول نص = العنوان، ثاني نص = subtitle، الباقي = content
                if (!$slideTitle) {
                    $slideTitle = $fullText;
                } elseif (!$slideSubtitle) {
                    $slideSubtitle = $fullText;
                } else {
                    $textBlocks[] = ['type' => 'text', 'value' => $fullText];
                }
            }

            // 5. حفظ الشريحة
            $newPres->slides()->create([
                'order'    => $slideIndex++,
                'category' => 'content',
                'type'     => $slideIndex === 1 ? 'start_slide' : 'content_slide',
                'content'  => [
        ...
        return response()->json([
            'status'  => false,
            'message' => 'Failed to parse PPTX: ' . $e->getMessage(),
        ], 422);
    }
}
}
